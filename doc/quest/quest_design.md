...
- **Inline-option pitfall (fixed)**: `optionKey(id, key, ...)` stores its 2nd argument *verbatim* as the option text with no lang registration, and `SimpleDialogOption.display()` is `Component.translatable(text())` — so passing a raw button string where the key belongs bakes the English sentence into the dialog data ("Mark me again.") and it can never be keyed/translated. The legacy `start`/`startRaid`/`follow`/`complete` helpers used to do this for their `start/accept`, `start/reject`, `follow_up/end`, `complete/reject`, `complete/handover` sub-buttons; the §7 audit surfaced 50 such raw buttons across Reimu/Marisa, now fixed to route through `option(id, text, ...)` (which registers a key via `optionText`). Rule: pass only already-keyed strings to `optionKey` (shared/daily keys, `byeKey`); any user-visible button string goes through `option(...)`.
