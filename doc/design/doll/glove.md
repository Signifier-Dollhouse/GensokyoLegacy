...
`DollGloveMode` enum (ids for the wheel) + seven small handler classes under `content/item/glove/mode/`, following `BorderUmbrellaMode`/`UmbrellaMode`:
...
| 1 | `HEAL_MARK` | `Items.GOLDEN_CARROT` | `use()`: toggle the heal mark on the cached ray-trace target (control.md §5–6). Any living entity, same 48-block range. |
| 2 | `VOLLEY` | `GLItems.STAR` (star danmaku, the starlight-hexbrew star) | `use()`: `issueIteration(player, target, REGULAR_ATTACK)` on the cached target. |
| 3 | `SUPER` | explosive hexbrew | `use()`: `issueOneTime(player, target, SUPER_ATTACK)` — one **random** available doll. |
| 4 | `SUICIDE` | `Items.TNT` | `use()`: `issueOneTime(player, target, SUICIDE)` — one **random** available doll. |
| 5 | `STOP` | `Items.SHIELD` | `use()`: stop **all summoned** dolls — `doll.actions.stop()` + clear `iteration`. Strays and block-hosted dolls untouched. |
| 6 | `EDITOR` | doll icon | `use()`: open the loadout editor for the cached target doll — works out of reach (no distance check on the menu side). Non-doll target → `not_doll` message. |

All modes always available (no anvil unlocks). Attack/mark/editor modes act on the cached ray-trace target (§2); empty cache → `no_target` message.
...
- Client pre-filters the ray trace by held mode, mirroring the server checks: editor only caches dolls (ownership is server-side only — the owner UUID is not synced — so the client filters by type and the server enforces owner/creative); volley/super/suicide only cache valid attack targets (not self, not player-owned, not any doll, attackable, non-spectator); summon caches nothing (it summons/recalls globally). Heal-mark/stop take any living entity.
- Glowing marker: client-side only, only while the player holds the glove — a mixin on the glow check (`LivingEntity`/`Entity#isCurrentlyGlowing` or the render path) returns true for the cached UUID, and a `getTeamColor` mixin tints the outline in the held mode's color (`DollGloveMode.glowColor`: aqua summon, green heal-mark, red attacks, gray stop, gold editor). Declared in `gensokyolegacy.mixins.json`. No server sync involved.
- Server `use()` consumes the last-synced UUID: re-validates alive, within 48 blocks. Attack modes additionally require a valid attack target — no allies (`OwnableEntity` owned by the holder, which covers their dolls and pets), no dolls at all, attackable and non-spectator. Heal-mark and editor take anything (editor needs dolls). Stale/missing cache → `no_target` message.
- Heal-mark mode uses the same target pipeline (any living entity, 48 blocks).

## 2b. Edit-mode hover overlay (ModularGolems mirror)

- `DollGloveOverlay` (client GUI layer above the crosshair, registered in `GLClient`): while the glove is held in `EDITOR` mode with no screen open, hovering a doll (vanilla crosshair `EntityHitResult`) shows its name plus the loadout in the menu's cross arrangement (`DollLoadoutTooltip`/`DollClientLoadoutTooltip`, slot frames + ghost icons from the menu texture atlas). Display-only; opening still goes through `use()`.

## 2c. Attack-mode status sidebar

- `DollAttackStatusOverlay` (client GUI layer above the crosshair, registered in `GLClient`): while the glove is held in volley/super/suicide mode with no screen open, the right side lists every summoned doll in ledger order as its doll item icon (tint + health bar, built from the doll entity's synced data) with a status frame — hidden for invalid (no valid weapon for this attack in either hand) or untracked dolls, white idle, yellow preparing (ticket held, behavior not started), red attacking, green done (already fired in the live volley). Display-only.
...
- `DollGloveSelectionListener` — `extends IItemSelector implements WheelAdaptor.Provider`, id `gensokyolegacy:doll_glove`, `static register()` → `IItemSelector.register(INSTANCE)` (§4); `test(stack)` = `instanceof DollGloveItem`; `getIndex`/`getList`/`swap` read and write the `DOLL_GLOVE_MODE` component.
- `DollGloveModeWheel` — `PersistentWheel<DollGloveModeEntry>` over the 7 modes; `DollGloveModeEntry` renders the mode icon; `select(index)` sends `DollGloveSelectPacket(0, index)`.
...
- `DollGloveSelectPacket(int wheel, int index)` — server-side mode switch on the held glove (mirror `BorderUmbrellaSelectPacket`).
...
- No other packets required: all commands run server-side, the ledger is server-side, and slot sync rides the existing entity data (loadout.md §3).
