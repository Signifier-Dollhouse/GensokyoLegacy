...
public class GLFluids {
	...
	public enum Hexbrew {
		MUNDANE("mundane_hexbrew", 0xFF9E9E9E),
		EXPLOSIVE("explosive_hexbrew", 0xFFE8453C),
		MIASMA("miasma_hexbrew", 0xFF7A4BA1),
		STARLIGHT("starlight_hexbrew", 0xFFFFF7AE),
		HYPHAE("hyphae_hexbrew", 0xFFD98E3A);

		public final FluidEntry<GLHexFluid> fluid;
		public final ItemEntry<HexbrewBottleItem> bottle;

		Hexbrew(String id, int color) {
			...
			bottle = GensokyoLegacy.REGISTRATE.item(id + "_bottle", p -> new HexbrewBottleItem(fluid::getSource, p.stacksTo(16)))
					.model((ctx, pvd) -> pvd.generated(ctx, pvd.modLoc("item/hexbrew/" + ctx.getName())))
